...
* **채팅 UI 모던화 (CSS Injection):** 
  * 사용자가 요청한 대로 **사용자 입력은 왼쪽, AI 답변은 오른쪽**으로 배치되도록 커스텀 CSS 적용.
  * 각 메시지 블록에 둥근 모서리와 모던한 배경색(파란빛/어두운 회색) 및 포인트 테두리(Border)를 추가하여 가독성 강화.
* **설명 문구 개선:** 앱 상단의 설명을 구어체로 부드럽고 친절하게 변경 ("마인크래프트 공식 위키와 커뮤니티의 꿀팁들을 모두 모아...").
* **사이드바 정보 하단 고정:** 데이터 소스 및 AI 모델 정보를 사이드바 맨 아래로 내리고 폰트 크기를 줄여 깔끔하게 정리.